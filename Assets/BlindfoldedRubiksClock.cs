...
public class BlindfoldedRubiksClock : MonoBehaviour
{
...
#pragma warning disable 0414
    private string TwitchManualCode = "Blindfolded Rubik%E2%80%99s Clock";
...
#pragma warning restore 0414
    ...
    void Start()
    {
        ...
        Bomb.OnBombExploded += delegate
        {
            ...
            {
                Debug.LogFormat("[Blindfolded Rubik’s Clock #{0}] Actions performed before bomb exploded:{1}", _moduleId, FormatActions());
            }
        };
        ...
        for (int count = 0; count < _moves.Count; count++)
        {
            ...
            Debug.LogFormat("[Blindfolded Rubik’s Clock #{0}] Moves to solve, move {1}:", _moduleId, count + 1);
            Debug.LogFormat("[Blindfolded Rubik’s Clock #{0}] - Lit clock: {1}. Lit pin: {2}.", _moduleId, _toDir9[move.LitClock], _toDir4[move.LitPin]);
            Debug.LogFormat("[Blindfolded Rubik’s Clock #{0}] - {1}, {2} ({3}). {4}, {5} ({6}).", _moduleId,
            ...
            Debug.LogFormat("[Blindfolded Rubik’s Clock #{0}] - Big square: {1}, Small square: {2}.", _moduleId, _toDir9[move.BigSquare], _toDir4[move.SmallSquare]);
            Debug.LogFormat("[Blindfolded Rubik’s Clock #{0}] - Change pins {1}, Rotate gear {2} for {3}.", _moduleId,
    ...
    private void Scramble(int numMoves)
    {
        ...
        Debug.LogFormat(
            "[Blindfolded Rubik’s Clock #{0}] Initial modifications: Action 1: row {1}. Amount 1: row {2}. Action 2: row {3}. Amount 2: row {4}",
    ...
    private void ResetModule()
    {
        ...
        Debug.LogFormat("[Blindfolded Rubik’s Clock #{0}] Actions performed before reset:{1}", _moduleId, FormatActions());
    ...
    private string FormatActions()
    {
        ...
        return String.Join(string.Format("\n[Blindfolded Rubik’s Clock #{0}] ", _moduleId), msgs.ToArray());
    }
    ...
    private void CheckState()
    {
        ...
        if (_clocks.SequenceEqual(Enumerable.Repeat(0, 18).ToArray()))
        {
            ...
            Debug.LogFormat("[Blindfolded Rubik’s Clock #{0}] Actions performed to solve:{1}", _moduleId, FormatActions());
        }
